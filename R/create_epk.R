...
create_epk <- function(
  ...
  # Also verify equal file counts per marker — same unique sample_ids does not
  # guarantee same ncol when replicate counts differ across markers.
  marker_file_counts <- vapply(unique_markers, function(m) {
    sum(!is.na(bw_metadata$marker) & bw_metadata$marker == m)
  }, integer(1))
  names(marker_file_counts) <- unique_markers

  if (length(unique(marker_file_counts)) > 1) {
    details <- paste(
      names(marker_file_counts), "[n_files=", marker_file_counts, "]",
      collapse = "\n"
    )
    stop(
      "Markers have different numbers of BigWig files (likely unequal replicate counts per sample).\n",
      "All assay matrices must have the same ncol. Check which samples are missing replicates:\n",
      "  table(bw_metadata$marker, bw_metadata$sample_id)\n",
      "Use 'markers_to_include' to restrict to balanced markers, or supply 'sample_metadata' ",
      "to manually assign files to columns.\n\n",
      "File counts per marker:\n", details
    )
  }
